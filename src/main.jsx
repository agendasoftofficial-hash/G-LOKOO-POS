  ...
  return <div className="login-screen">
    <div className="login-card">
      ...
      <form onSubmit={submit}>
        ...
        <label>Password<input type="password" required value={password} onChange={e=>setPassword(e.target.value)} placeholder="Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢"/></label>
  ...
  function addPhoneToCart(phone){
    ...
    setCart(c=>{
      ...
      return [...c,{
        ...
        meta:[phone.storage,phone.ram,phone.color].filter(Boolean).join(" Ã¢â‚¬Â¢ ")
      }];
    });
    setPhoneImei("");
  }
      ...
      <div className="phone-lookup">
        ...
        {searchedPhones.length>0&&<div className="imei-results">{searchedPhones.slice(0,5).map(p=>
          <button key={p.id} onClick={()=>addPhoneToCart(p)} className="imei-result">
            <div><b>{p.brand} {p.model}</b><small>{p.imei_1} {p.imei_2?`Ã¢â‚¬Â¢ ${p.imei_2}`:""} {p.storage?`Ã¢â‚¬Â¢ ${p.storage}`:""}</small></div>
      ...
      <div className="cart-lines">
        ...
        cart.map(i=><div className="cart-line" key={i.id}>
          <div className="line-info">
            <b>{i.name}</b>
            {i.kind==="phone"
              ? <small className="mono">IMEI: {i.imei}{i.meta?` Ã¢â‚¬Â¢ ${i.meta}`:""}</small>
...
function Imei({phones,reload,canManage=true}){
  ...
  return <section className="content">
    ...
    <div className="table-panel"><table><thead><tr><th>Phone</th><th>IMEI 1</th><th>IMEI 2</th><th>Cost</th><th>Selling</th><th>Status</th></tr></thead><tbody>
      {filtered.map(p=><tr key={p.id}><td><b>{p.brand} {p.model}</b><small>{[p.storage,p.ram,p.color].filter(Boolean).join(" Ã¢â‚¬Â¢ ")}</small></td><td className="mono">{p.imei_1}</td><td className="mono">{p.imei_2||"Ã¢â‚¬â€"}</td><td>{money(p.cost)}</td><td>{money(p.selling_price)}</td><td><span className={`badge ${p.status==="In Stock"?"ok":"warn"}`}>{p.status}</span></td></tr>)}
